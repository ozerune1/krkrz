...
#include "tjsScriptBlock.h"   // GetName(), SrcPosToLine()
#include "tjsGlobalStringMap.h"

// 旧独自デバッガ (Win32 WM_COPYDATA + TScriptDebuggerForm) との連携コードは
// 撤去済み。VSCode DAP ベースの新デバッガは tjsDebuggerHook.cpp /
// tjsDebuggerSymbols.cpp および (将来追加予定の) DebuggerCore に分割される。
...
public:
	...
	{
		out.clear();
		// 上位 frame から順に走査。InTry frame (try/catch の実装人工 frame) は
		// スキップする (Context は親 frame と同じなので重複表示を避ける)。
		for (int top = (int)Stack.size() - 1; top >= 0; --top) {
			const tTJSStackRecord& rec = Stack[top];
			if (rec.InTry) continue;
			...
			// filename: Block->GetName()
			if (auto* blk = rec.Context->GetBlock()) {
				const tjs_char* nm = blk->GetName();
				if (nm) f.filename = tjs_string(nm);
			}

			// funcname: "ClassName.funcName" or just "funcName" or "(top)"
			ttstr cls = rec.Context->GetClassName();
			const tjs_char* fname = rec.Context->GetName();
			ttstr disp;
			if (!cls.IsEmpty()) {
				...
				disp = ttstr(TJS_W("(top)"));
			}
			f.funcname = disp.AsStdString();

			// line: codeoffset → srcpos → line (1-based)
			if (rec.CodeBase && rec.CodePtr) {
				tjs_int offset = (tjs_int)(*rec.CodePtr - rec.CodeBase);
				if (offset >= 0) {
					tjs_int srcpos = rec.Context->CodePosToSrcPos(offset);
					if (srcpos >= 0 && rec.Context->GetBlock()) {
						f.line = (int)(rec.Context->GetBlock()->SrcPosToLine(srcpos) + 1);
					}
				}
			}
...
{
	if (TJSStackTracer) TJSStackTracer->GetFrames(out);
	else out.clear();
}
//---------------------------------------------------------------------------
